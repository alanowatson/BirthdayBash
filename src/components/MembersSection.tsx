...
export default function MembersSection({ members }: { members: Member[] }) {
  ...
  return (
    <section id="members" className="py-24 px-6 border-t" style={{ borderColor: 'var(--gold-soft)' }}>
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          <p className="section-label mb-4">The Guest List</p>
          <h2 className="font-display text-5xl md:text-6xl gold-gradient mb-3">Who&#39;s Coming</h2>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {visible.map((member, i) => (
                ...
                >
                  ...
                  {member.bio && <p className="text-xs text-text-dim mb-2 line-clamp-1">{member.bio}</p>}
                  {member.is_referee && <p className="text-xs text-sky">Referee</p>}
                </a>
              ))}
            </div>
            ...
            {overflow > 0 && (
              <div className="text-center mt-10">
                <a href="/members" className="text-gold hover:underline text-sm uppercase tracking-widest">
