...
export default async function MemberPage({ params, searchParams }: Props) {
  ...
  const hasRsvpd = (rsvpData ?? []).some((r) => r.status === 'attending' || r.status === 'maybe');
    ...
    <main className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-24">
        <div className="w-full max-w-md text-center">
          ...
          {/* Profile info — only shown once they've RSVP'd */}
          {hasRsvpd && (member.known_for || member.fun_fact || member.obsession) && (
